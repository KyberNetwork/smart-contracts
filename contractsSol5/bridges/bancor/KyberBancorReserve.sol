...
contract KyberBancorReserve is IKyberReserve, Withdrawable2, Utils4 {
    uint256 constant ETH_BNT_DECIMALS = 18;
    ...
    constructor(
        ...
        require(
            _bancorNetwork != address(0),
            "constructor: bancorNetwork address is missing"
        );
        require(
            _kyberNetwork != address(0),
            "constructor: kyberNetwork address is missing"
        );
        require(
            _bancorToken != address(0),
            "constructor: bancorToken address is missing"
        );
    ...
    function trade(
        ...
        require(
            ...
            "trade: src or dest must be ETH"
        );
        require(
            srcToken == bancorToken || destToken == bancorToken,
            "trade: src or dest must be BNT"
        );
        ...
        require(
            ...
            "trade: doTrade function returns false"
        );
    ...
    function setKyberNetwork(address _kyberNetwork) public onlyAdmin {
        require(
            _kyberNetwork != address(0),
            "setKyberNetwork: kyberNetwork address is missing"
        );
    ...
    function setBancorContract(address _bancorNetwork) public onlyAdmin {
        require(
            _bancorNetwork != address(0),
            "setBancorContract: bancorNetwork address is missing"
        );

        require(
            bancorToken.approve(address(bancorNetwork), 0),
            "setBancorContract: can not reset allowance"
        );

        bancorNetwork = IBancorNetwork(_bancorNetwork);
        require(
            bancorToken.approve(_bancorNetwork, 2**255),
            "setBancorContract: can not approve token"
        );
    ...
    function setNewEthBntPath(
        IERC20[] memory _ethToBntPath,
        IERC20[] memory _bntToEthPath
    ) public onlyAdmin {
        require(
            _ethToBntPath.length != 0,
            "setNewEthBntPath: path should have some elements"
        );
        require(
            _bntToEthPath.length != 0,
            "setNewEthBntPath: path should have some elements"
        );
        ...
        require(
            destQty > 0,
            "setNewEthBntPath: no rate from eth to bnt with this path"
        );

        (destQty, ) = bancorNetwork.getReturnByPath(_bntToEthPath, amount);
        require(
            destQty > 0,
            "setNewEthBntPath: no rate from bnt to eth with this path"
        );
    ...
    function doTrade(
        ...
        if (validate) {
            ...
            if (srcToken == ETH_TOKEN_ADDRESS)
                require(
                    msg.value == srcAmount,
                    "doTrade: msg value is not correct for ETH trade"
                );
            else
                require(
                    msg.value == 0,
                    "doTrade: msg value is not correct for token trade"
                );
        }
        ...
        if (srcToken != ETH_TOKEN_ADDRESS) {
            ...
            require(
                ...
                "doTrade: collect src token failed"
            );
        }
        ...
        require(
            ...
            "doTrade: user expected amount must be greater than 0"
        );
        uint256 destAmount;

        if (srcToken == ETH_TOKEN_ADDRESS) {
            destAmount = bancorNetwork.convert2.value(srcAmount)(
                path,
                srcAmount,
            ...
            destAmount = bancorNetwork.claimAndConvert2(
                path,
                srcAmount,
                userExpectedDestAmount,
                address(0),
        ...
        require(
            destAmount >= userExpectedDestAmount,
            "doTrade: dest amount is lower than expected amount"
        );
        ...
        if (destToken == ETH_TOKEN_ADDRESS) {
            ...
            require(
                ...
                "doTrade: transfer back dest token failed"
            );
        }
    ...
    function getConversionPath(IERC20 src, IERC20 dest)
        public
        view
