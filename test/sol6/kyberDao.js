...
contract('nimbleDao', function(accounts) {
  before("one time init", async() => {
    ...
    await NIMToken.transfer(victor, mulPrecision(1000000));
    await NIMToken.transfer(mike, mulPrecision(1000000));
    await NIMToken.transfer(loi, mulPrecision(1000000));
    await NIMToken.transfer(poolMaster, mulPrecision(1000000));
    await NIMToken.transfer(poolMaster2, mulPrecision(1000000));
  });
  ...
  const setupSimpleStakingData = async() => {
    // approve tokens
    await NIMToken.approve(stakingContract.address, mulPrecision(1000000), {from: victor});
    await NIMToken.approve(stakingContract.address, mulPrecision(1000000), {from: mike});
    await NIMToken.approve(stakingContract.address, mulPrecision(1000000), {from: loi});
    await NIMToken.approve(stakingContract.address, mulPrecision(1000000), {from: poolMaster});
    await NIMToken.approve(stakingContract.address, mulPrecision(1000000), {from: poolMaster2});
  ...
  const setupTokenWithSupply = async(supply) => {
    NIMToken = await TestToken.new("test token", 'tst', 18, {from: accounts[0]});

    let totalSupply = await NIMToken.totalSupply();
    let burnAmount = totalSupply.sub(new BN(supply));

    await NIMToken.burn(burnAmount, {from: accounts[0]});

    Helper.assertEqual(supply, await NIMToken.totalSupply(), "total supply is invalid");

    await NIMToken.transfer(victor, supply.div(new BN(5)));
    await NIMToken.transfer(mike, supply.div(new BN(5)));
    await NIMToken.transfer(loi, supply.div(new BN(5)));
    await NIMToken.transfer(poolMaster, supply.div(new BN(5)));
    await NIMToken.transfer(poolMaster2, supply.div(new BN(5)));
  }

  const resetSetupForNIMToken = async() => {
    NIMToken = await TestToken.new("test token", 'tst', 18, {from: accounts[0]});
    await NIMToken.transfer(victor, mulPrecision(1000000));
    await NIMToken.transfer(mike, mulPrecision(1000000));
    await NIMToken.transfer(loi, mulPrecision(1000000));
  ...
  const simpleSetupToTestThreshold = async(mikeStake, victorStake, loiStake, percentage) => {
    ...
    let burnAmount = (await NIMToken.totalSupply()).sub(new BN(totalSupply));
    await NIMToken.burn(burnAmount, {from: accounts[0]});

    await NIMToken.transfer(mike, initMikeStake);
    await NIMToken.transfer(victor, initVictorStake);
    await NIMToken.transfer(loi, initLoiStake);
  ...
  describe("#Handle Withdrawal tests", () => {
    ...
    it("Test handle withdrawal updates correct data after withdraw - with delegation", async function() {
      ...
      await nimbleDao.vote(1, 2, {from: victor});
      // note: Loi already withdraw 10 NIM
    ...
    it("Test handle withdrawal should revert when sender is not staking", async function() {
      nimbleDao = await nimbleDaoContract.new(
        ...
        NIMToken.address, minCampPeriod,
  ...
  describe("#Submit Campaign tests", () => {
    it("Test submit campaign returns correct data after created", async function() {
      ...
      let totalSupply = await NIMToken.INITIAL_SUPPLY();
      ...
      for(let id = 0; id <= 2; id++) {
        ...
        Helper.assertEqual(totalSupply, data.totalNIMSupply, "total supply is incorrect");
        ...
        // burn NIM to reduce total supply value
        await NIMToken.burn(mulPrecision(1000000));
    ...
    it("Test submit new campaign for next epoch, data changes as expected", async function() {
      ...
      Helper.assertEqual(await NIMToken.totalSupply(), data.totalNIMSupply, "total supply is incorrect");
  ...
  describe("#Cancel Campaign tests", () => {
    ...
    it("Test cancel campaign should update correct data after cancelled", async function() {
      ...
      for(let id = 0; id < 2; id++) {
        ...
        Helper.assertEqual(campData.totalNIMSupply, 0, "camp details should be deleted");
    ...
    it("Test cancel campaign correctly for network fee camp", async function() {
      ...
      Helper.assertEqual(campData.totalNIMSupply, await NIMToken.totalSupply(), "camp details should be correct");
      ...
      await NIMToken.burn(mulPrecision(100));
      await submitNewCampaign(nimbleDao,
  ...
  describe("#Conclude Campaign tests", () => {
    ...
    it("Test get winning option total supply is 0", async function() {
      ...
      await NIMToken.burn(totalSupply, {from: accounts[0]});
  ...
  describe("#Constructor tests", () => {
    it("Test correct data is set after deployment", async function() {
      ...
      Helper.assertEqual(await nimbleDao.NIMToken(), NIMToken.address, "NIM token is wrong");
      ...
      Helper.assertEqual(await nimbleDao.NIMToken(), await stakingContract.NIMToken(), "NIM token is wrong");
