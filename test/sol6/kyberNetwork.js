...
contract('NimbleNetwork', function(accounts) {
    ...
    describe("should test events declared in network contract", async() => {
        ...
        before("setup contracts ", async() => {
            ...
            KNC = await TestToken.new("Nimble network crystal", "KNC", 18);
            feeHandler = await FeeHandler.new(NimbleDao.address, proxyForFeeHandler.address, tempNetwork.address, KNC.address, burnBlockInterval, NimbleDao.address);
    ...
    describe("test list reserves", async function() {
        ...
        before("global setup", async function(){
            ...
            //init feeHandler
            token = await TestToken.new("Nimble network crystal", "KNC", 18);
        });
    ...
    describe("test trades with MockNimbleDao", async() => {
        before("initialise NimbleDao, network and reserves", async() => {
            ...
            // init feeHandler
            KNC = await TestToken.new("Nimble network crystal", "KNC", 18);
            feeHandler = await FeeHandler.new(NimbleDao.address, proxyForFeeHandler.address, network.address, KNC.address, burnBlockInterval, NimbleDao.address);
        ...
        describe("Test maxDestAmount reverse calculation edge cases", async() => {
            ...
            before("setup contracts", async() => {
                ...
                // init feeHandler
                let KNC = await TestToken.new("Nimble network crystal", "KNC", 18);
                let feeHandler = await FeeHandler.new(NimbleDao.address, mockNetwork.address, mockNetwork.address, KNC.address, burnBlockInterval, NimbleDao.address);
    ...
    describe("test fee handler integrations with 1 mock and 1 fpr", async() => {
        ...
        before("setup, add and list reserves", async() => {
            ...
            await NimbleDao.setNetworkFeeBps(networkFeeBps);
            KNC = await TestToken.new("Nimble network crystal", "KNC", 18);
            [network, storage] = await nwHelper.setupNetwork(NimbleNetwork, networkProxy, KNC.address, NimbleDao.address, admin, operator);
    ...
    describe("test maximum trade quantity", async() => {
        ...
        before("init token balance", async() => {
            ...
            let feeHandler = await FeeHandler.new(NimbleDao.address, network.address, network.address, KNC.address, burnBlockInterval, NimbleDao.address);
