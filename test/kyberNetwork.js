...
//    });
    ...
    it("should test listing and unlisting new pair for new reserve. see rate changes. token to eth. as expected.", async function () {
        ...
        //first unlist token
        await network.listPairForReserve(reserve3.address, testedToken, true, true, false);
    ...
    it("should test listing and unlisting new pair for new reserve. see rate changes. token to token. as expected.", async function () {
        ...
        try {
             await network.tradeWithHint(user1, listedToken.address, amount, testedToken, user2, maxDestAmount,
        ...
        try {
             await network.tradeWithHint(user1, testedToken, amount, listedToken.address, user2, maxDestAmount,
                0 ,walletId, 0, {from:networkProxy});
             assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
...
//            log("expected trade value: " + expectedDestAmount)
    ...
    it("enhance step func: setup reserve for enhance step function", async function () {
        ...
        await network.addReserve(reserve4.address, true);
        for (let i = 0; i < numTokens; i++) {
            // unlist token for reserve 1 and 2
            await network.listPairForReserve(reserve1.address, tokenAdd[i], true, true, false);
            await network.listPairForReserve(reserve2.address, tokenAdd[i], true, true, false);
            await network.listPairForReserve(reserve4.address, tokenAdd[i], true, true, true);
        }
        network.addReserve(reserve1.address, false);
    ...
    it("enhance step func: should only enable new reserve perform sell and check: balances changed as expected.", async function () {
        let tokenInd = 2;
        let token = tokens[tokenInd]; //choose some token
        let amountTwei = 1030 * 1;
        let user = accounts[9];
