...
//        console.log("current block: " + currentBlock);
        ...
        uniqueToken= await TestToken.new("uinque", "unq", 4);
...
//        console.log(baseSellRate2)
        ...
        let baseBuyUnique = [precisionUnits.mul(40).valueOf()];
        let baseSellUnique = [precisionUnits.div(40).valueOf()];
//        log(uniqueAddArr + "  " + baseBuyUnique + "  " + baseSellUnique)
    ...
    it("should init network and 2 reserves and set all reserve data including balances", async function () {
        ...
        await uniqueToken.transfer(reserve3.address, 10000000000);
...
//        console.log("init user balance: " + user2InitBalance + " post balance: " + user2PostBalance + " diff " + user2PostBalance*1 - user2InitBalance*1);
    ...
    it("should test listing and unlisting new pair for new reserve. see rate changes. as expected.", async function () {
        let listedToken = tokenAdd[2];
        ...
        let rates = await network.getExpectedRate(listedToken, testedToken, amount);
        assert.equal(0, rates[0].valueOf());
        rates = await network.getExpectedRate(testedToken, listedToken, amount);
        assert.equal(0, rates[0].valueOf());

        //list token. buy (eth to token)
        await network.listPairForReserve(reserve3.address, testedToken, true, true, true);
        rates = await network.getExpectedRate(listedToken, testedToken, amount);
        log("rates[0].valueOf() " + rates[0].valueOf())
