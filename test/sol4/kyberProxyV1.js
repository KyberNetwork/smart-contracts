...
contract('nimbleProxyV1', function(accounts) {
    before("one time global init", async function () {
        ...
        NIM = await TestToken.new("nimble krystal", "NIM", 18);
        NIMAddress = NIM.address;
        ...
        feeHandler = await FeeHandler.new(nimbleDao.address, networkProxyV1.address, network.address, NIM.address, burnBlockInterval, nimbleDao.address);
    ...
    describe("MaliciousNetwork + nimbleProxyV1", async () => {
        ...
        before("init smart malicious network and set all contracts and params", async () => {
            networkProxyV1 = await NetworkProxyV1.new(admin);
            [maliciousNetwork, tempStorage] = await nwHelper.setupNetwork(MaliciousNetwork, networkProxyV1.address, NIM.address, nimbleDao.address, admin, operator);
    ...
    describe("MaliciousNetwork2 + Proxy1", async() => {
        before("init malicious network returning wrong actual dest, and set all contracts and params", async function () {
            networkProxyV1 = await NetworkProxyV1.new(admin);
            [maliciousNetwork2, tempStorage] = await nwHelper.setupNetwork(MaliciousNetwork2, networkProxyV1.address, NIM.address, nimbleDao.address, admin, operator);
    ...
    describe("GenerousNetwork + Proxy1", async() => {
        before("init 'generous' network with trade reverse direction, could result in overflow.", async function () {
            ...
            networkProxyV1 = await NetworkProxyV1.new(admin);
            [generousNetwork, tempStorage] = await nwHelper.setupNetwork(GenerousNetwork, networkProxyV1.address, NIM.address, nimbleDao.address, admin, operator);
    ...
    describe("NetworkNoMaxDest + Proxy1", async() => {
        before("init network with no max dest check. set all contracts and params", async function () {
            networkProxyV1 = await NetworkProxyV1.new(admin);
            [networkNoMaxDest, tempStorage] = await nwHelper.setupNetwork(NetworkNoMaxDest, networkProxyV1.address, NIM.address, nimbleDao.address, admin, operator);
