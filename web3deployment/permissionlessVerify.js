...
//        myLog(0, 0, blockCode);
    ...
    let NIMPerEthBaseRatePrecision = await Reserve.methods.NIMPerEthBaseRatePrecision().call();
    myLog(0, 0, "NIMPerEthBaseRatePrecision " + NIMPerEthBaseRatePrecision.valueOf() + " == " + getAmountTokens(NIMPerEthBaseRatePrecision.valueOf(), 18) + " tokens.");

    let contracts = await Reserve.methods.contracts().call();
    myLog(0, 0, "token " + (await a2n(contracts[0].valueOf(), 1)));
    myLog(0, 0, "NIMToken " + (await a2n(contracts[1].valueOf(), 1)));
    ...
    let depositNIMEvents = await Reserve.getPastEvents("NIMFeeDeposited", {fromBlock: 0, toBlock: 'latest'});
    let makersDictNIMAmount = [];

    for(let i = 0; i < depositNIMEvents.length; i++) {
        let maker = depositNIMEvents[i].returnValues.maker;
        if(makersDictNIMAmount[maker] == undefined) makersDictNIMAmount[maker] = web3.utils.toBN(0);
        makersDictNIMAmount[maker] = (web3.utils.toBN(depositNIMEvents[i].returnValues.amount)).add(makersDictNIMAmount[maker]);
    };

    for(let maker in makersDictNIMAmount) {
        myLog(0, 0, "Maker: " + maker + ": NIM deposited:  " + getAmountTokens(makersDictNIMAmount[maker].valueOf(), 18));
    }
}
