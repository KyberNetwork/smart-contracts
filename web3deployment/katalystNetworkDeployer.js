...
let feeHandlerPermissions;
let networkPermissions;
let proxyPermissions;

//misc variables needed for contracts deployment
let maxGasPrice = new BN(50).mul(new BN(10).pow(new BN(9)));
let negDiffInBps = new BN(15);
let burnBlockInterval = new BN(100);
...
function parseInput( jsonInput ) {
    ...
    feeHandlerPermissions = jsonInput.permission["FeeHandler"];
    networkPermissions = jsonInput.permission["Network"];
    proxyPermissions = jsonInput.permission["Proxy"];

    maxGasPrice = new BN(jsonInput["max gas price"].toString());
    negDiffInBps = new BN(jsonInput["neg diff in bps"].toString());
    burnBlockInterval = new BN(jsonInput["burn block interval"].toString());
...
async function deployTradeLogicContract(output) {
    if (tradeLogicAddress == "") {
        ...
        tradeLogicContract = new web3.eth.Contract(JSON.parse(
            output.contracts["KyberTradeLogic.sol:KyberTradeLogic"].interface), tradeLogicAddress
        );
    }
}
...
async function deployNetworkContract(output) {
    if (networkAddress == "") {
        ...
        networkContract = new web3.eth.Contract(JSON.parse(
            output.contracts["KyberNetwork.sol:KyberNetwork"].interface), networkAddress
        );
    }
}
...
async function deployProxyContract(output) {
    if (proxyAddress == "") {
        ...
        proxyContract = new web3.eth.Contract(JSON.parse(
            output.contracts["KyberNetworkProxy.sol:KyberNetworkProxy"].interface), proxyAddress
        );
    }
}
...
async function deployFeeHandlerContract(output) {
    if (feeHandlerAddress == "") {
        ...
        feeHandlerContract = new web3.eth.Contract(JSON.parse(
            output.contracts["FeeHandler.sol:FeeHandler"].interface), feeHandlerAddress
        );
    }
}
