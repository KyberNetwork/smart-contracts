...
abstract contract KyberHintHandler is IKyberHint, Utils5 {
        ...
        pure
    {
        if (reserveIds.length != 0 || splits.length != 0)
            throwHintError(HintErrors.NoneEmptyDataError);
    }
    ...
    function verifyData(
        ...
        if (tradeType == TradeType.BestOfAll) {
            if (reserveIds.length != 0 || splits.length != 0) return HintErrors.NoneEmptyDataError;
        }
    ...
    function throwHintError(HintErrors error) internal pure {
        if (error == HintErrors.NoneEmptyDataError) revert("reserveIds and splits must be empty");
